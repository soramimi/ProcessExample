...
src/                             - ライブラリ本体
  ...
  ProcessConPtyWithWorker.h/cpp  - ConPTY を別 worker exe に分離する実装
...
- `AbstractPtyProcess`: 疑似端末型プロセス。共有状態 (mutex_, cond_, change_dir_, completed_fn_, user_data_, output_queue_, output_vector_, stdout_bytes_, stderr_bytes_) を基底クラスが直接保持し、非 virtual の `write_output()` / `pop_output()` / `set_change_dir()` / `set_completion_callback()` / `notify_completed()` を提供する。
...
| `ProcessWinConPty` | `src/ProcessWinConPty.h/cpp` | `AbstractPtyProcess` | `BasicProcessWinConPTY` | ConPTY 直接所有版。`output_vector` 既定オン。stderr は空扱い。`set_no_window()` / `set_options()` を持つ。`wait()` はブロッキング中に state mutex を保持しないため、待機中の `write_input()` / `stop()` が機能する。`stop()` は `terminate()` で子を終了させてから回収する。 |
...
| `ProcessConPtyWithWorker` | `src/ProcessConPtyWithWorker.h/cpp` | `AbstractPtyProcess` | `BasicProcessWin` + `conpty-worker.exe` | ConPTY を別プロセスへ分離する構成。Qt Creator など外側の疑似端末との干渉回避が目的。`stop()` は worker を強制終了してから回収する。 |
...
2. worker 側は `ProcessConPtyWithWorker::run_worker(argc, argv)` で worker mode を判定する。worker mode の識別タグは `--conpty-worker--`。
...
なお `src/ProcessConPtyWithWorker.cpp` は `sampleapp/base64.h` と `sampleapp/misc.h` を include しており、ライブラリ側が sampleapp 側ヘッダに依存している (両 .pro の INCLUDEPATH で解決している)。
...
- 通常ビルドでは `git --version` を使い、6 方式を順に全実行する (固定 `select` は廃止): `main_basic_win` (BasicProcessWin) → `main_basic_win_conpty` (BasicProcessWinConPTY) → `main_win` (ProcessWin) → `main_win_conpty` (ProcessWinConPty) → `main_win_conpty_with_worker` (worker 分離) → `main_winpty` (winpty)。
- `main_win_conpty_with_worker()` は `misc::find_windows_openssh()` で Windows OpenSSH を探し、`git -c core.sshCommand=<ssh> --version` を実行。`wait_for_output("Are you sure you want to continue connecting")` を検出したら `write_input("no\n", 3)` を送るデモが有効になっている。
...
- `src/ProcessConPtyWithWorker.cpp` が `sampleapp/base64.h` / `sampleapp/misc.h` に依存しており、ライブラリが自己完結していない。
